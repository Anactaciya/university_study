...
std::ostream& operator<<(std::ostream& stream, const PointCloud& pc) {
	stream << "Point cloud (" << pc.getTypeString() << "): " << std::endl;
	for (size_t i = 0; i < pc.size(); ++i) {
		for (size_t j = 0; j < pc.pointSize(); ++j) {
			stream << pc.points()[i * pc.pointSize() + j] << " ";
		}
		stream << std::endl;
	}
	return stream;
...
int main() {
	try {
		...
		// delete out; не нужно из-за умного указателя
